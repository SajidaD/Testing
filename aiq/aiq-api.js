/*! aiq-api-mock - Level6 - v1.2.0- - 2015-03-02 */

var aiq = aiq || {
    version: '6',

    /**
     * Returns the index of an element chosen by passing it through
     * the specified qualifier on the given list.
     *
     * @note                    Should not be covered by the unittest
     *                          as exists in the Mocked Api only
     *
     * @param {Array} array        array on which to find the matchinbg
     *                             element
     * @param {Function} qualifier function to qualify elements of the
     *                             array, should return true for the
     *                             element which index to return
     * @return {Number}            index of the qualified element or -1
     *                             if no element got qualified
     */
    _indexOf: function(array, qualifier) {
        var result = -1;
        array.some(function(object, index) {
            if (qualifier(object)) {
                result = index;
                return true;
            }
    ...
    /**
     * Genetes a pseudo-UUID string with given seed.
     *
     * @note                    Should not be covered by the unittest
     *                          as exists in the Mocked Api only
     *
     * @param  {Number} seed    Seed to be used to generate an UUID
     * @return {String}         UUID string
     */
    _newUUID: function(seed) {
        return Date.now().toString(16) + '_' + seed;
    },

    /**
     * Creates an error with given name and parent.
     *
     * @note                    Should not be covered by the unittest
     *                          as exists in the Mocked Api only
     *
     * @param {String} name     Error name
     * @param {Error}  Parent   Parent error
     * @return {Error}          New error class
     */
    _createError: function(name, Parent) {
    ...
    _testURL: function(url, success, failure) {
        ...
        head.onreadystatechange = function() {
            if (head.readyState === XMLHttpRequest.DONE) {
                if (head.status === 404) {
     ...
     * @return {Void}
     */
    _failWithMessage: function (callbacks, ErrorClass, message, otherFlags) {
        if (typeof callbacks.failure === 'function') {
            var error = new ErrorClass(message);
            if (typeof otherFlags === 'object') {
                Object.keys(otherFlags).forEach(function(key) {
                    error[key] = otherFlags[key];
                })
            }
    ...
    _doesSourceMatchPattern: function (source, pattern) {
        ...
        } else if ((aiq._typeOf(pattern) === 'array') &&
                   ((aiq._typeOf(source) === 'number') || (aiq._typeOf(source) === 'boolean'))) {
            // texas range(r)
            if (pattern.length !== 2) {
...
(function() {
    ...
    function checkComplete() {
        ...
        aiq.AIQError = aiq._createError('AIQError', Error);
        aiq.NotFoundError = aiq._createError('NotFoundError', aiq.AIQError);
        aiq.IdNotFoundError = aiq._createError('IdNotFoundError', aiq.NotFoundError);
        aiq.NameNotFoundError = aiq._createError('NameNotFoundError', aiq.NotFoundError);
        aiq.ResourceNotFoundError = aiq._createError('ResourceNotFoundError', aiq.NotFoundError);
        aiq.InvalidArgumentError = aiq._createError('InvalidArgumentError', aiq.AIQError);
        aiq.ConnectionError = aiq._createError('ConnectionError', aiq.AIQError);
        aiq.ContainerError = aiq._createError('ContainerError', aiq.AIQError);
        ...
        aiq.datasync.createDocument('_launchable', {
            ...
            success: function(doc) {
                ...
                aiq._addEventListener = document.addEventListener;
                document.addEventListener = function(event, listener, useCapture) {
                    if (event === 'aiq-ready') {
                        listener();
                    } else {
                        aiq._addEventListener.call(document, event, listener, useCapture);
                    }
                };
...
(function () {
    ...
    if (!aiq.datasync) {
        ...
        aiq.datasync.getDocument = function (id, settings) {
            ...
            if (typeof id !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Identifier not specified');
                return;
            }

            if (typeof settings.success === 'function') {
                var index = aiq._indexOf(aiq.datasync._documents, function(doc) { return doc._id === id; });
                if (index === -1) {
                    aiq._failWithMessage(settings, aiq.IdNotFoundError, 'Document not found');
                    return;
                }
                
                settings.success(aiq.datasync._documents[index]);
            }
        };
        ...
        aiq.datasync.createDocument = function (type, fields, settings) {
            ...
            if (typeof type !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Type not specified');
                return;
            }

            if ((! fields) || (typeof fields !== 'object')) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Fields not specified');
                return;
            }

            var id = fields._id || aiq._newUUID(aiq.datasync._documents.length);
            if (aiq.datasync._documents.some(function(doc) { return doc._id === id; })) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Duplicate document identifier');
                return;
            }
        ...
        aiq.datasync.updateDocument = function (id, fields, settings) {
            ...
            doc._id = id;
            doc._type = aiq.datasync._documents[index]._type;
            doc._status = 'updated';
            aiq.datasync._documents[index] = doc;
        ...
        aiq.datasync.deleteDocument = function (id, settings) {
            ...
            var type = aiq.datasync._documents[index]._type;
            aiq.datasync._documents.splice(index, 1);
            ...
            if (typeof settings.success === 'function') {
                try {
                    ...
                    aiq.datasync._documents.forEach(function (doc) {
                        if (doc._type === type) {
                            if (!settings.filter || aiq._doesSourceMatchPattern(doc, settings.filter)) {
                    ...
                    aiq._failWithMessage(settings, aiq.InvalidArgumentError, error.message);
                }
            }
        };
        ...
        aiq.datasync.getAttachment = function (id, name, settings) {
            ...
            if (typeof name !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Name not specified');
                return;
            }
            ...
            if (typeof settings.success === 'function') {
                ...
                aiq.datasync.getDocument(id, {
                    success: function () {
                        ...
                        if (! attachment) {
                            aiq._failWithMessage(settings, aiq.NameNotFoundError, 'Attachment not found');
                            return;
                        }
        ...
        var storeAttachment = function (id, name, descriptor, status, callback) {
            var attachments = aiq.datasync._attachments[id] || {};
            attachments[name] = {
        ...
        aiq.datasync.createAttachment = function (id, descriptor, settings) {
            ...
            if ((! descriptor) || (typeof descriptor !== 'object')) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Descriptor not specified');
                return;
            }

            if (typeof descriptor.contentType !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Content type not specified');
                return;
            }

            if (typeof descriptor.resourceUrl !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Resource not specified');
                return;
            }
            ...
            aiq.datasync.getDocument(id, {
                success: function () {
                    aiq._testURL(descriptor.resourceUrl, function () {
                        var attachments = aiq.datasync._attachments[id] || {};
                        var name = descriptor.name || aiq._newUUID(Object.keys(attachments).length);
                        if (attachments.hasOwnProperty(name)) {
                            aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Duplicate attachment name');
                            return;
                        }
                        
                        storeAttachment(id, name, descriptor, 'created', settings.success);
                    }, function () {
                        aiq._failWithMessage(settings, aiq.ResourceNotFoundError, 'Resource not found');
                    });
                },
                failure: settings.failure
            });
        ...
        aiq.datasync.updateAttachment = function (id, name, descriptor, settings) {
            ...
            aiq.datasync.getDocument(id, {
                success: function () {
                    ...
                    aiq._testURL(descriptor.resourceUrl, function () {
                        storeAttachment(id, name, descriptor, 'updated', settings.success);
                    }, function () {
                        aiq._failWithMessage(settings, aiq.ResourceNotFoundError, 'Resource not found');
                    });
                },
                    ...
                    success: function () {
                        ...
                        var result = Object.keys(attachments).map(function (name) {
                            return {
                                ...
                                state: 'available',
                                status: attachments[name].status
                            };
                        });
        ...
        if ((json) && (typeof json === 'object') && (json.hasOwnProperty('documents'))) {
            json.documents.forEach(function (doc) {
                var id = doc._id || Date.now().toString(16) + '_' + aiq.datasync._documents.length;
                if (aiq.datasync._documents.some(function(document) { return document._id === id; })) {
                    return;
                }
            
                var document = aiq._cloneObject(doc, '^_');
                document._id = id;
                document._type = doc._type;
                document._status = doc._status || 'synchronized';
                aiq.datasync._documents.push(document);
            });
...
(function () {
    ...
    if (! aiq.client) {
        ...
        aiq.client.navbar.addButton = function (properties, settings) {
            ...
            if (! properties.hasOwnProperty('image')) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Image not specified');
                return;
            }

            if (typeof properties.onClick !== 'function') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Action not specified');
                return;
            }
            ...
            if ((properties.visible) && (countVisibleButtons() === MAX_BUTTONS)) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Too many visible buttons');
                return;
            }

            aiq._testURL(properties.image, function() {
                var buttonDescriptors = getButtonDescriptors();
                var descriptor = {
                    id: aiq._newUUID(buttonDescriptors.length),
        ...
        aiq.client.navbar.updateButton = function (id, properties, settings) {
            ...
            var buttonDescriptors = getButtonDescriptors();
            var index = aiq._indexOf(buttonDescriptors, function(button) { return button.id === id; });
            if (index === -1) {
                aiq._failWithMessage(settings, aiq.IdNotFoundError, 'Button not found');
                return;
            }
            
            var descriptor = buttonDescriptors[index];

            var index = aiq._indexOf(buttonDescriptors, function(descriptor) { return descriptor.id === id; });
            if (index === -1) {
                aiq._failWithMessage(settings, aiq.IdNotFoundError, 'Button not found');
                return;
            }
            ...
            if (typeof descriptor.image === 'string') {
                aiq._testURL(descriptor.image, function() {
                    refreshNavbar(buttonDescriptors);
                    ...
                    aiq._failWithMessage(settings, aiq.ResourceNotFoundError, 'ResourceNotFound');
                });
            } else {
                refreshNavbar(buttonDescriptors);
        ...
        aiq.client.navbar.getButton = function (id, settings) {
            ...
            if (typeof settings.success === 'function') {
                ...
                settings.success(aiq._cloneObject(buttonDescriptors[index]));
            }
        };
    ...
    if ((json) && (typeof json === 'object') &&
        ...
        var currentUser = {
            _id: user._id || aiq._newUUID(Date.now()),
...
(function() {
    ...
    if (! aiq.context) {
        ...
        var getContext = function(type, name, success, failure) {
            aiq.datasync.getDocuments(type, {
                success: function(docs) {
                    if ((docs.length === 1) && (docs[0].hasOwnProperty(name))) {
                        success(docs[0][name]);
                    } else {
                        aiq._failWithMessage({failure: failure}, aiq.NameNotFoundError, 'Context not found');
                    }
                },
                failure: failure
            });
                ...
                getContext(
                    ...
                    function(apps) {
                        if (apps.hasOwnProperty(key)) {
                            ...
                            aiq._failWithMessage(settings, aiq.NameNotFoundError, 'Context not found');
                        }
                    },
                    settings.failure,
                    settings.error);
...
(function() {
    ...
    if (!aiq.directcall) {
        ...
        var handleOldInput = function(args, inputs, settings) {
            ...
            if (typeof found.errorCode === 'number') {
                aiq._failWithMessage(
                    settings,
                    aiq.ConnectionError,
                    'Invalid response from AIQ Server',
                    {
            ...
            } else if (typeof found.data !== 'undefined') {
                settings.success(found.data);
            } else {
                aiq._failWithMessage(settings, aiq.ContainerError, 'Invalid mock file');
            }
            return true;
        };
        ...
        var call = function(method, args, settings) {
            ...
            if (typeof args.endpoint !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Endpoint not specified');
                return;
            }
            ...
            if (((method === 'get') ||
                ...
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Body not allowed');
                return;
            }

            if ((args.hasOwnProperty('body')) && (args.hasOwnProperty('resourceUrl'))) {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Body not allowed');
                return;
            }
...
(function() {
    ...
    if (! aiq.messaging) {
        ...
        aiq.messaging.getAttachment = function(id, name, callbacks) {
            ...
            if (typeof id !== 'string') {
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Identifier not specified');
                return;
            }

            if (typeof name !== 'string') {
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Name not specified');
                return;
            }
            ...
            if (typeof callbacks.success === 'function') {
                ...
                aiq.messaging.getMessage(id, {
                    success: function() {
                        ...
                        var attachment = attachments[name];
                        if (! attachment) {
                            aiq._failWithMessage(callbacks, aiq.NameNotFoundError, 'Attachment not found');
                            return;
                        }
                ...
                try {
                    ...
                    aiq.messaging._serverOriginatedMessages.forEach(function(message) {
                        if ((message.type === type) &&
                            ((! settings.filter) || (aiq._doesSourceMatchPattern(message, settings.filter)))) {
                            var copy = aiq._cloneObject(message);
                            copy.relevant = false;
                            if (! settings.withPayload) {
                                delete copy.payload;
                            }
                            messages.push(copy);
                        }
        ...
        aiq.messaging.getMessage = function(id, settings) {
            ...
            var index = aiq._indexOf(aiq.messaging._serverOriginatedMessages, function(message) { return message._id === id; });
            if (index === -1) {
                aiq._failWithMessage(settings, aiq.IdNotFoundError, 'Message not found');
                return;
            }

            if (typeof settings.success === 'function') {
                var copy = aiq._cloneObject(aiq.messaging._serverOriginatedMessages[index]);
                copy.relevant = false;
        ...
        aiq.messaging.deleteMessage = function(id, settings) {
            ...
            aiq.messaging._serverOriginatedMessages.splice(index, 1);
        ...
        aiq.messaging.markMessageAsRead = function(id, settings) {
            ...
            aiq.messaging._serverOriginatedMessages[index].read = true;
            ...
            if (['message-received', 'message-updated', 'message-expired'].indexOf(event) !== -1) {
                ...
                if (typeof settings._id === 'string') {
                    ...
                    if (event === 'message-received') {
                        aiq.messaging._serverOriginatedMessages.some(function(message) {
                            if (message._id === settings._id) {
                                settings.callback(message._id);
                                return true;
                            }
                            return false;
                        });
        ...
        aiq.messaging.sendMessage = function(descriptor, attachmentsOrCallbacks, callbacksOrNull) {
            var attachments;
            var callbacks;
            if ((attachmentsOrCallbacks instanceof Array) || (typeof callbacksOrNull === 'object')) {
            ...
            if (typeof descriptor !== 'object') {
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Descriptor not specified');
                return;
            }

            if (! descriptor.hasOwnProperty('payload')) {
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Payload not specified');
                return;
            }

            if (typeof descriptor.destination !== 'string') {
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Destination not specified');
                return;
            }
            ...
            var sendMessage = function() {
                ...
                var message = {
                    _id: aiq._newUUID(aiq.messaging._clientOriginatedMessages.length),
                    destination: descriptor.destination,
                    created: Date.now(),
                    state: 'queued'
                };
                if (descriptor.expectResponse) {
            ...
            var testAttachments = function() {
                if (attachments.length === 0) {
                    ...
                    if (typeof attachment.contentType !== 'string') {
                        aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Content type not specified');
                        return;
                    }

                    if (typeof attachment.resourceUrl !== 'string') {
                        aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Resource not specified');
                        return;
                    }
                    ...
                    aiq._testURL(attachment.resourceUrl, function() {
                        ...
                        aiq._failWithMessage(callbacks, aiq.ResourceNotFoundError, 'Resource not found');
                    });
                }
            };
            ...
            if (isUnique(attachments, 'name')) {
                ...
                aiq._failWithMessage(callbacks, aiq.InvalidArgumentError, 'Duplicate attachment name');
            }
        };
        ...
        aiq.messaging.getMessageStatus = function(id, callbacks) {
            ...
            var index = aiq._indexOf(aiq.messaging._clientOriginatedMessages, function(message) { return message._id === id; });
            if (index === -1) {
                aiq._failWithMessage(callbacks, aiq.IdNotFoundError, 'Message not found');
                return;
            }

            if (typeof callbacks.success === 'function') {
                callbacks.success(aiq._cloneObject(aiq.messaging._clientOriginatedMessages[index]));
            }
        };
        ...
        aiq.messaging.getMessageStatuses = function(destination, settings) {
            ...
            if (typeof destination !== 'string') {
                aiq._failWithMessage(settings, aiq.InvalidArgumentError, 'Destination not specified');
                return;
            }
                ...
                root.forEach(function(message) {
                    if (typeof message.type === 'string') {
                        aiq.messaging._serverOriginatedMessages.push({
                            _id: message._id || aiq._newUUID(aiq.messaging._serverOriginatedMessages.length),
                            type: message.type,
...
(function () {
    ...
    if (!aiq.storage) {
        ...
        aiq.storage.getDocument = function (id, settings) {
            ...
            var index = aiq._indexOf(aiq.storage._documents, function(doc) { return doc._id === id; });
            if (index === -1) {
                aiq._failWithMessage(settings, aiq.IdNotFoundError, 'Document not found');
                return;
            }
            ...
            if (typeof settings.success === 'function') {
                return settings.success(aiq._cloneObject(aiq.storage._documents[index]));
            }
        };
        ...
        aiq.storage.createDocument = function (type, fields, settings) {
            ...
            var id = fields._id || aiq._newUUID(aiq.storage._documents.length);
            ...
            var document = aiq._cloneObject(fields, '^_');
            document._id = fields._id || aiq._newUUID(aiq.storage._documents.length);
            document._type = type;
        ...
        aiq.storage.updateDocument = function (id, fields, settings) {
            ...
            var doc = aiq._cloneObject(fields, '^_');
            doc._id = id;
            doc._type = aiq.storage._documents[index]._type;
            aiq.storage._documents[index] = doc;
        ...
        aiq.storage.deleteDocument = function (id, settings) {
            ...
            aiq.storage._documents.splice(index, 1);
